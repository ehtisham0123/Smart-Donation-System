  ...
  try {
    ...
    await db.query(sql, [email], function (err, result) {
      if (result.length > 0) {
        ...
        (async () => {
          var sql =
            "INSERT INTO `patients` ( email,firstname,lastname,fathername,gender,age,contact,avatar,housenumber,streetnumber,city,state,postalcode,country,latitude,longitude,file_name,file_type,cnic,familyincome,bothparrentsdied,eldersiblingage,home, doctor,hospital,disease,expensis) VALUES (?)";
          var values = [
            email,
            firstname,
            lastname,
