...
function Patients({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getUsersData = async () => {
      await axios
        .get(`${process.env.React_App_Url}/admin/patients/`,{
          headers: {
            token: token,
          },
        })
  ...
  const searchPatient = async (name) => {
    setLoading(true);
    await axios
      .get(`${process.env.React_App_Url}/admin/patients/${name}`,{
          headers: {
            token: token,
          }
        })
    ...
    <div id="content" className="p-4">
      <div className="card-body">
        ...
        <div className="row d-flex align-items-center justify-content-between  mr-1">
          ...
          <Link to={`${match.url}/new-patients`}>
            <button className="btn btn-outline-dark mr-1">
              <i className="fa fa-user-plus"></i> New Patients
            </button>
          </Link>
        </div>
        ...
        >
          ...
          <tbody>
              ...
              {currentPatients.map((patient) => (
                <PatientsTableRow match={match} patient={patient} deletePatient={deletePatient}/>
              ))}
          </tbody>
            )}
        </table>
