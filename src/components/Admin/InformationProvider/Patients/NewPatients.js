...
import NewPatientsTableRow from "./NewPatientsTableRow";
...
function Patients({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getUsersData = async () => {
      await axios
        .get(`${process.env.React_App_Url}/admin/patients/new-patients/`,{
          headers: {
            token: token,
          },
        })
  ...
  const deletePatient = async (id) => {
    await axios.delete(`${process.env.React_App_Url}/admin/patients/${id}`,{
          headers: {
            token: token,
          },
        }).then((res) => {
      const newPatients = patient.filter((patient) => patient.id !== id);
      setPatients(newPatients);
    });
  };

  const addPatient = async (id) => {
    await axios.get(`${process.env.React_App_Url}/admin/patients/new/${id}`,{
          headers: {
            token: token,
          },
        }).then((res) => {
  ...
  const searchPatient = async (name) => {
    setLoading(true);
    await axios
      .get(`${process.env.React_App_Url}/admin/patients/new-patients/${name}`,{
          headers: {
            token: token,
          }
        })
        ...
        >
          ...
          <tbody>
              ...
              {currentPatients.map((patient) => (
                <NewPatientsTableRow match={match} patient={patient} deletePatient={deletePatient} addPatient={addPatient}/>
              ))}
          </tbody>
            )}
        </table>
