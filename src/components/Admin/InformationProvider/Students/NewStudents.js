...
import NewStudentsTableRow from "./NewStudentsTableRow";
...
function Students({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getUsersData = async () => {
      await axios
        .get(`${process.env.React_App_Url}/admin/students/new-students/`,{
          headers: {
            token: token,
          },
        })
  ...
  const addStudent = async (id) => {
    await axios.get(`${process.env.React_App_Url}/admin/students/new/${id}`,{
          headers: {
            token: token,
          },
        }).then((res) => {
  ...
  const searchStudent = async (name) => {
    setLoading(true);
    await axios
      .get(`${process.env.React_App_Url}/admin/students/new-students/${name}`,{
          headers: {
            token: token,
          }
        })
        ...
        >
          ...
          <tbody>
              ...
              {currentStudents.map((student) => (
                <NewStudentsTableRow match={match} student={student} deleteStudent={deleteStudent} addStudent={addStudent}/>
              ))}
          </tbody>
            )}
        </table>
